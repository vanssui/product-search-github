import { API_VERSION } from '../types/contracts.js';
      ...
      try {
        ...
        if (status?.completed) {
          if (status.state === 'failed') {
            throw new ApiError(
              status.error?.code || 'WRITE_FAILED',
              status.error?.message || 'Операция записи не выполнена.',
    ...
    try {
      if (method === 'GET') {
        ...
        response = await fetch(this.baseUrl, {
          ...
          body: JSON.stringify({ apiVersion: API_VERSION, action, requestId, ...payload }),
          signal: controller.signal
        });
      }
    } catch (error) {
